const undergrad = {
    title: "Computer Science",
    organization: "University of North Carolina, Chapel Hill",
    location: "Chapel Hill, NC",
    date: "August 2018 - May 2022",
    details: ["Pursuing a double major in Computer Science and Statistics, minoring in Astronomy",
        "Sponsorship Committee Lead for HackReality, first XR-focused hackathon in Southeast region.",
        "ospitality Chair for Triangle Asian-American Student Conference.", 
        "Events Chair for UNC-Chapel Hill branch of SolidariTee"]
};

const hs = {
    organization: "William G. Enloe High School",
    location: "Raleigh, NC",
    date: "August 2014 - June 2018",
    details: ["President/Founder of Enloe Film Society",
        "Chief Editor of Enloe Literary Organization",
        "Member of varsity QuizBowl team, WebMasters, and local NPO Food Ark"]
};

export const education = [undergrad, hs];